...
and officially leaves it up to each B<MDCDBI>-implementing library to
...
some DBMSs may consider C<Integer> to be a I<subtype> of I<Fraction> (C<42>
...
Each B<MDCDBI>-implementing library is not required to support every one of
...
application process using the B<MDCDBI>-implementing library.
...
defined to give an integer result when the divisor is zero, and so a Muldis
D function for integer division could be defined to result in an C<Excuse>
...
For a B<MDCDBI>-implementing library that natively has a concept of
...
=head2 Excuse_Neg_Inf

The singleton type value C<Excuse_Neg_Inf> represents the type-agnostic
concept of negative infinity, an I<orderable> value that sorts I<before>
all other values in the type system.  This value is also an I<excuse> that
says the result of some operation is infinite in the negative direction.

=head2 Excuse_Pos_Inf

The singleton type value C<Excuse_Pos_Inf> represents the type-agnostic
concept of positive infinity, an I<orderable> value that sorts I<after> all
other values in the type system.  This value is also an I<excuse> that says
the result of some operation is infinite in the positive direction.
...
=head2 Excuse_No_Such_Index

The singleton type value C<Excuse_No_Such_Index> represents the
I<undefined> result of attempting to use a member index I<I> of
C<Array>/etc value I<V> while assuming incorrectly that I<V> already has a
member whose index is I<I>.
