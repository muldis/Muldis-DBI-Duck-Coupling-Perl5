...
This document is hereby proposed as the official successor to the Perl 5
L<DBI> library, and thus is formally proposed to take the namespace C<DBI2>
in place of its current C<Muldis::DBI::Duck_Coupling> name.
...
Similarly, it is proposed that any versions of existing C<DBD::*> modules
adapted to this C<DBI2> would by default have their corresponding
C<DBD2::*> names, though they can also be free to pick new names of their
choosing, as C<DBI2> would not place any requirements on implementers names.
